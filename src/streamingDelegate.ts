...
export class EufyCameraStreamingDelegate implements CameraStreamingDelegate {
    ...
    async handleSnapshotRequest(
      ...
      // streamData.videostream.pipe(converter.createInputStream({ f: videoFormat }));

      // // get jpegs and pipe them to joiner process
      // converter.createOutputStream({ f: 'mjpeg' }).pipe(joinerInput, { end: false });
      // this.platform.eufyClient.stopStationLivestream(this.device.getSerial());
      // await converter.run();

      // const bufs = [];
      // joinerInput.on('data', (d)=> {
      //   imageBuffer = Buffer.concat([imageBuffer, d]);
      // });
      // joinerInput.on('end', ()=> {
